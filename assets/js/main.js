...
(function ($) {
  ...
  mm.add("(min-width: 1200px)", () => {
    if (document.querySelectorAll(".hero-area").length > 0) {
      ...
      const bigtextColor = isDarkMode ? "#FFFFFF" : "#faaa00";
  ...
  // Hover Image Effects
  function initHoverImageEffects() {
    // Create hover image elements
    const brazilImage = document.createElement('div');
    brazilImage.className = 'hover-image';
    brazilImage.innerHTML = '<img src="assets/icons/TUCANO@4x.png" alt="Brazil">';
    document.body.appendChild(brazilImage);

    const momentsImage = document.createElement('div');
    momentsImage.className = 'hover-image';
    momentsImage.innerHTML = '<img src="assets/icons/LUAR@4x.png" alt="Moments">';
    document.body.appendChild(momentsImage);

    // Get hover elements
    const brazilElement = document.querySelector('.hover-brazil');
    const momentsElement = document.querySelector('.hover-moments');

    // Brazil hover effect
    if (brazilElement) {
      brazilElement.addEventListener('mouseenter', function(e) {
        brazilImage.classList.add('active');
      });

      brazilElement.addEventListener('mouseleave', function(e) {
        brazilImage.classList.remove('active');
      });

      brazilElement.addEventListener('mousemove', function(e) {
        const rect = brazilImage.getBoundingClientRect();
        const x = e.clientX - rect.width / 2;
        const y = e.clientY - rect.height / 2;
        
        brazilImage.style.left = x + 'px';
        brazilImage.style.top = y + 'px';
      });
    }

    // Moments hover effect
    if (momentsElement) {
      momentsElement.addEventListener('mouseenter', function(e) {
        momentsImage.classList.add('active');
      });

      momentsElement.addEventListener('mouseleave', function(e) {
        momentsImage.classList.remove('active');
      });

      momentsElement.addEventListener('mousemove', function(e) {
        const rect = momentsImage.getBoundingClientRect();
        const x = e.clientX - rect.width / 2;
        const y = e.clientY - rect.height / 2;
        
        momentsImage.style.left = x + 'px';
        momentsImage.style.top = y + 'px';
      });
    }
  }

  // Initialize hover effects when document is ready
  $(document).ready(function() {
    initHoverImageEffects();
  });
